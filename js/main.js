(function() {
      ...
      var searchData = [
        { title: 'Car Emergency Kit', desc: 'Complete guide to roadside preparedness', url: '/car-emergency-kit', category: 'Kits', icon: 'ðŸš—' },
        { title: 'Fire Emergency Kit', desc: 'Home fire safety and escape essentials', url: '/fire-emergency-kit', category: 'Kits', icon: 'ðŸ”¥' },
        { title: '72-Hour Survival Kit', desc: 'Three-day survival kit build guide', url: '/guides/72-hour-kit', category: 'Guides', icon: 'â±' },
        { title: 'Home Emergency Kit', desc: 'Household emergency preparedness', url: '/guides/home-emergency-kit', category: 'Guides', icon: 'ðŸ ' },
      ...
      function performSearch(query) {
        ...
        var allData = searchData.concat(faqSearchData);
        ...
        Object.keys(grouped).forEach(function(cat) {
          ...
          grouped[cat].forEach(function(item) {
            var icon = item.icon ? '<div class="search-result-icon"><span style="font-size:16px">' + item.icon + '</span></div>' :
              '<div class="search-result-icon"><svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg></div>';
            html += '<a href="' + item.url + '" class="search-result-item">' + icon +
